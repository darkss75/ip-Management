...
// File-based data store for persistence
class FileStore {
    constructor() {
        this.dataDir = path.join(__dirname, 'storage');
        this.countriesFile = path.join(this.dataDir, 'countries.json');
        this.blockedIPsFile = path.join(this.dataDir, 'blockedIPs.json');
    ...
    loadData() {
        // 국가 데이터 로드
        if (fs.existsSync(this.countriesFile)) {
            try {
        ...
        if (fs.existsSync(this.blockedIPsFile)) {
            try {
                const data = fs.readFileSync(this.blockedIPsFile, 'utf8');
                this.blockedIPs = JSON.parse(data);
            } catch (error) {
        ...
        if (fs.existsSync(this.counterFile)) {
            try {
                ...
                this.idCounter = counter.value || 1;
            } catch (error) {
        ...
        console.log(`Loaded ${this.countries.length} countries and ${this.blockedIPs.length} blocked IPs`);
    }
    ...
    saveCountries() {
        try {
            fs.writeFileSync(this.countriesFile, JSON.stringify(this.countries, null, 2));
        } catch (error) {
    ...
    saveBlockedIPs() {
        try {
            fs.writeFileSync(this.blockedIPsFile, JSON.stringify(this.blockedIPs, null, 2));
        } catch (error) {
    ...
    saveCounter() {
        try {
            fs.writeFileSync(this.counterFile, JSON.stringify({ value: this.idCounter }));
        } catch (error) {
    ...
    createBlockedIP(data) {
        ...
        this.saveBlockedIPs();
        this.saveCounter();
        this.saveCountries(); // 국가별 카운트 업데이트 저장
